...
// Variables
volatile int Direccion = 0;  // Dirección del viento (0-359)
...
float VelocidadViento = 0.0; // Velocidad del viento en m/s
...
void loop() {
  ...
  VelocidadViento = getVelocidad();
  
  // Mostrar la dirección y velocidad del viento en el puerto serie
  mostrarVelocidadYDireccion(velocidad, direccion);
...
int getDireccion() {
  ...
  Direccion = map(ValorPot, 0, 1023, 0, 359); 
  Direccion = (Direccion + CorreccionDir + 3) % 360; // Corregir offset y precisión de 5°
  return Direccion;
}
...
float getVelocidad() {
  ...
  VelocidadViento = ContadorRPM * RPMaMS[indice];
...
void mostrarVelocidadYDireccion(float velocidad, int direccion) {
